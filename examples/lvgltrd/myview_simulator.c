...
// Rtemp * adc =  ((4096 - adc) * R1) = 4096*R1 - adc * R1;
// (Rtemp + R1) * adc = 4096 R1
// adc = 4096*R1 / (Rtemp + R1)
static uint32_t CalcAdcRawValue( uint32_t Rtemperature)
{
    const int R1 = 729;
    const int adc_max = 4096;
    return adc_max * R1 / (Rtemperature + R1);
}

static void * simulator_thread(void * arg)
{
    int32_t temperature = 10;
    struct adc_msg_s adc_dat;
    struct sensor_gyro gyro_dat;
    uint8_t Relay = 0;
    int ret = nx_mkfifo(FILEPATH_GYRO, 0777, BUFSIZE);
    if(ret != OK)
    {
        gerr("ERROR creating FIFO: %i", errno);
    }
    ret = nx_mkfifo(FILEPATH_ADC, 0777, BUFSIZE);
    if(ret != OK)
    {
        gerr("ERROR creating FIFO: %i", errno);
    }
    ret = nx_mkfifo( FILEPATH_RELAY, 0777, 1);
    if(ret != OK)
    {
        gerr("ERROR creating FIFO: %i", errno);
    }
    int flags = O_WRONLY | O_TRUNC ;
    int f_gyro = open(FILEPATH_GYRO, flags);
    int f_adc = open(FILEPATH_ADC, flags);
    int f_relay = open(FILEPATH_RELAY, O_RDONLY);
    if(f_adc < 0 ||f_gyro < 0)
    {
        _err("ERROR opening FIFO: %i", errno);
    }
    int err;
    for(;;)
    {
        ...
        {
            // no data written
            gwarn("simulator cannot write to ADC FIFO pipe\n");
        }
        err = write(f_gyro, &gyro_dat, sizeof(gyro_dat));
        if(err == 0)
        {
            // no data written
            gwarn("simulator cannot write to GYRO FIFO pipe\n");
        }
        usleep(100000);
        err = read(f_relay, &Relay, sizeof(Relay));
        if(err == 0)
        {
            // no data read
            gwarn("simulator cannot read from RELAY FIFO pipe\n");
        }
        if(Relay)
        {
            // relay is switched on --> increase temperature
            if(temperature < 110)
                temperature+=1;
        }
        else
...
{
    pthread_t thread;
    int ret = pthread_create(&thread, NULL, simulator_thread, (void *) NULL);
    if (ret != OK)
    {
